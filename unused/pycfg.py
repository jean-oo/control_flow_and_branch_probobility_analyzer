...
class CFGNode(dict):
    ...
    def lineno(self):  # get line number
        # return self.ast_node.lineno if hasattr(self.ast_node, 'lineno') else 0
        if astunparse.unparse(self.ast_node).strip() == "stop":
            return -1
        elif hasattr(self.ast_node, 'lineno'):
...
class PyCFG:
    ...
    def walk(self, node, myparents):
        ...
        # print(node.__class__.__name__.lower())
    ...
    def on_if(self, node, myparents):
        if len(node.orelse) == 1 and type(node.orelse[0]) != ast.Return:
            _test_node = CFGNode(parents=myparents, ast=ast.parse('_elif: %s' % astunparse.unparse(node.test).strip()).body[0])
        else: 
    ...
    def on_elif(self, node, myparents):
        _test_node = CFGNode(parents=myparents, ast=ast.parse('_elif: %s' % astunparse.unparse(node.test).strip()).body[0])
        ast.copy_location(_test_node.ast_node, node.test)
        test_node = self.walk(node.test, [_test_node])
        g1 = test_node
